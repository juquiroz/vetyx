...
type MarcarNoShowOutput =
  | { ok: true; cita: Cita; mensaje: string }
  | { ok: false; error: string }

export async function marcarNoShow(input: FormData): Promise<MarcarNoShowOutput> {
  ...
  const permiso = verificarPermiso(usuario.rol, "citas", "marcar-no-show")
  ...
  if (citaActual.estado !== "confirmed") {
    return { ok: false, error: "Solo se puede marcar como no show una cita confirmada" }
  }
  ...
  return { ok: true, cita: citaActualizada as Cita, mensaje: "Cita marcada como no asistió" }
}
