...
export async function limpiarClinicaCompleta() {
  ...
  if (usuarios) {
    await Promise.allSettled(
      usuarios.map((u) => admin.auth.admin.deleteUser(u.id))
    )
  }

  await admin.from("usuarios").delete().eq("clinic_id", cid)
  await admin.from("clinicas").delete().eq("id", cid)
